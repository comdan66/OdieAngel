...
class OALineBot {
  ...
  public static function createLog ($source, $speaker, $event) {
    ...
    switch (true) {
      ...
      case $event instanceof ImageMessage:    $params = array_merge ($message_params, array ('file' => '')); return LogImage::transaction (function () use (&$log, $params) { return ($log = LogImage::create ($params)) && $log->putFile2S3 (); }) ? $log : null;
      case $event instanceof VideoMessage:    $params = array_merge ($message_params, array ('file' => '')); return LogVideo::transaction (function () use (&$log, $params) { return ($log = LogVideo::create ($params)) && $log->putFile2S3 (); }) ? $log : null;
      case $event instanceof StickerMessage:  $params = array_merge ($message_params, array ('package_id' => $event->getPackageId (), 'sticker_id' => $event->getStickerId ())); return LogSticker::transaction (function () use (&$log, $params) { return $log = LogSticker::create ($params); }) ? $log : null;
      case $event instanceof LocationMessage: $params = array_merge ($message_params, array ('title' => $event->getTitle (), 'address' => $event->getAddress (), 'latitude' => $event->getLatitude (), 'longitude' => $event->getLongitude ())); return LogLocation::transaction (function () use (&$log, $params) { return $log = LogLocation::create ($params); }) ? $log : null;
      case $event instanceof AudioMessage:    $params = array_merge ($message_params, array ('file' => '')); return LogAudio::transaction (function () use (&$log, $params) { return ($log = LogAudio::create ($params)) && $log->putFile2S3 (); }) ? $log : null;
      case $event instanceof FileMessage:     $params = array_merge ($message_params, array ('name' => $event->getFileName (), 'size' => $event->getFileSize (), 'file' => '')); return LogFile::transaction (function () use (&$log, $params) { return ($log = LogFile::create ($params)) && $log->putFile2S3 (); }) ? $log : null;
      case $event instanceof PostbackEvent:   $params = array ('source_id' => $source->id, 'speaker_id' => $speaker->id, 'reply_token' => $event->getReplyToken () ? $event->getReplyToken () : '', 'data' => $event->getPostbackData (), 'params' => $event->getPostbackParams () ? json_encode ($event->getPostbackParams ()) : '', 'timestamp' => $event->getTimestamp () ? $event->getTimestamp () : ''); return LogPostback::transaction (function () use (&$log, $params) { return $log = LogPostback::create ($params); }) ? $log : null;
      case $event instanceof FollowEvent:     $params = array ('source_id' => $source->id, 'reply_token' => $event->getReplyToken () ? $event->getReplyToken () : '', 'timestamp' => $event->getTimestamp () ? $event->getTimestamp () : ''); return LogFollow::transaction (function () use (&$log, $params) { return $log = LogFollow::create ($params); }) ? $log : null;
      case $event instanceof JoinEvent:       $params = array ('source_id' => $source->id, 'reply_token' => $event->getReplyToken () ? $event->getReplyToken () : '', 'timestamp' => $event->getTimestamp () ? $event->getTimestamp () : ''); return LogJoin::transaction (function () use (&$log, $params) { return $log = LogJoin::create ($params); }) ? $log : null;
      case $event instanceof UnfollowEvent:   $params = array ('source_id' => $source->id, 'timestamp' => $event->getTimestamp () ? $event->getTimestamp () : ''); return LogUnfollow::transaction (function () use (&$log, $params) { return $log = LogUnfollow::create ($params); }) ? $log : null;
      case $event instanceof LeaveEvent:      $params = array ('source_id' => $source->id, 'timestamp' => $event->getTimestamp () ? $event->getTimestamp () : ''); return LogLeaf::transaction (function () use (&$log, $params) { return $log = LogLeaf::create ($params); }) ? $log : null;
    }
  }
}
...
class OALineBotRichmenu {
  ...
  public static function HTTPClient () { return OALineBotRichmenu::$http === null ? (OALineBotRichmenu::$http = new OALineBotCurlHTTPClient (config ('line', 'channel', 'token'))) : OALineBotRichmenu::$http; }
