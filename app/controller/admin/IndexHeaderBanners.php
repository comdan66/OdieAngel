...
class IndexHeaderBanners extends AdminRestfulController {

  public function __construct () {
    parent::__construct ();
    $this->layout->with ('title', '首頁上方輪播')
                 ->with ('current_url', RestfulUrl::url ('admin/IndexHeaderBanners@index'));
  }
  ...
  public function index () {
    ...
    $total = IndexHeaderBanner::count ($where);
    $page  = Pagination::info ($total);
    $objs  = IndexHeaderBanner::find ('all', array (
    ...
    return $this->view->setPath ('admin/IndexHeaderBanners/index.php')
  ...
  public function add () {
    return $this->view->setPath ('admin/IndexHeaderBanners/add.php');
  }
  ...
  public function edit ($obj) {
    return $this->view->setPath ('admin/IndexHeaderBanners/edit.php');
  }
  ...
  public function destroy ($obj) {
    if ($error = IndexHeaderBanner::getTransactionError (function ($obj) { return $obj->destroy (); }, $obj))
  ...
  public function status ($obj) {
    ...
    if ($error = IndexHeaderBanner::getTransactionError ($transaction, $posts, $obj))
      return Output::json ($error, 400);
  ...
  public function sorts () {
    $validation = function (&$posts) {
      Validation::maybe ($posts, 'changes', '狀態', array ())->isArray ()->doArrayValues ()->doArrayMap (function ($t) {
        ...
        if (!$obj = IndexHeaderBanner::find ('one', array ('select' => 'id,sort', 'where' => Where::create ('id = ? AND sort = ?', $t['id'], $t['ori']))))
    ...
    if ($error = IndexHeaderBanner::getTransactionError ($transaction, $posts))
      return Output::json ($error, 400);
