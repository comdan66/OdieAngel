...
class OriAds extends AdminRestfulController {

  public function __construct () {
    parent::__construct ();
    $this->layout->with ('title', '原生廣告')
                 ->with ('current_url', RestfulUrl::url ('admin/OriAds@index'));
  }
  ...
  public function index () {
    ...
    return $this->view->setPath ('admin/OriAds/index.php')
                      ->with ('search', $search)
                      ->with ('pagination', implode ('', $page['links']));
  }

  public function add () {
    return $this->view->setPath ('admin/OriAds/add.php');
  }
  ...
  public function edit ($obj) {
    return $this->view->setPath ('admin/OriAds/edit.php');
  }
  ...
  public function status ($obj) {
    $validation = function (&$posts) {
      Validation::maybe ($posts, 'status', '狀態', OriAd::STATUS_OFF)->isStringOrNumber ()->doTrim ()->doRemoveHtmlTags ()->inArray (array_keys (OriAd::$statusTexts));
    };
    ...
    if ($error = OriAd::getTransactionError ($transaction, $posts, $obj))
      return Output::json ($error, 400);
