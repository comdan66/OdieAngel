...
<?php echo $search->setTableClomuns (
  ...
  Restful\Column::create ('圖示')->setWidth (50)->setClass ('oaips')->setTd (function ($obj) { return $obj->icon->toImageTag ('w100'); }),
  ...
  Restful\Column::create ('點擊次數')->setWidth (100)->setSort ('click_cnt')->setTd (function ($obj) { return '<a href="' . RestfulUrl::url ('admin/PvStores@index', $obj) .'">' . number_format ($obj->click_cnt) . '</a>'; }),

  Restful\EditColumn::create ('編輯')->setTd (function ($obj, $column) {
    return $column->addDeleteLink (RestfulUrl::destroy ($obj))
                  ->addEditLink (RestfulUrl::edit ($obj))->addLink (RestfulUrl::url ('admin/PvStores@index', $obj), array ('class' =>'icon-29')); }));
?>
</div>
